...
| **M3 4-seed ensemble, edge_buffer=25 (legacy)** | **0.533** | **yes** |
| **M3 4-seed ensemble, edge_buffer=200** | **0.565** | **yes** |
...
**On the comparison to RDP — characterized across seeds, not cherry-picked.** A single epoch is not evidence: LANL F1 across one model's 25 saved epochs is 0.461 ± 0.027 (range 0.404–0.509 at edge_buffer=25), so the original 0.509 headline was the *favourable end* of a noisy quantity on a 35-breakpoint test set. We therefore trained **four independent seeds** of the same recipe and evaluated each seed's deployment checkpoint (the one selected by SANTA-validation F1, independent of LANL). Two reporting conventions, both honest:

| | per-seed F1 (4 seeds) | 4-seed ensemble | vs RDP 0.519 |
|---|---|---|---|
| edge_buffer = 25 (legacy) | 0.524 ± 0.016 (0.506–0.545) | 0.533 | +0.014 |
| edge_buffer = 200 (UTR fix, §4.2, 0 TP loss) | **0.554 ± 0.013** (0.538–0.568) | **0.565** | **+0.046** |

The across-seed std (0.013–0.016) is half the within-seed epoch std, and **every individual seed beats RDP at edge_buffer=200** (min 0.538). The original v2 single seed (0.509 at eb=25) sat at the *low* end of the seed distribution. So the robust, variance-controlled conclusion is that M3 — a sequence-only detector — **exceeds classical RDP standalone on LANL**, by +0.014 (conservative, legacy convention, ensemble) to +0.046 (200-bp convention, ensemble). The recommended deployment is the 4-seed ensemble (`m3_eval_ensemble.py`); the edge-buffer convention is justified independently of LANL (no true BP within 300 bp of an end).

Two caveats stated plainly. (i) **The panel is small — 35 breakpoints across 4 CRFs** — so a +0.01 to +0.05 margin is real but not large; the value of the result is "a sequence-only detector reaches parity-to-better with the full RDP suite," not the exact decimal. (ii) **All F1s use the test-panel-optimal global threshold** (thr=0.15), for both M3 and the RDP figure, so the comparison is apples-to-apples; a threshold fixed on SANTA-validation (the deployment-honest choice) would be marginally lower and is a preprint follow-up.
...
This is packaged as a genuine two-head detector (`m3_gated_detector.py`, `M3GatedDetector`): a frozen BP head ("where is the breakpoint") and an aux head emitting a graded [0,1] confidence ("is this a within-distribution recombinant I should trust"), with the BP peaks gated on the aux head's confidence. This is exactly the decoupling the v4 goal specified — *where* separated from *whether*, BP gated on aux confidence rather than the BP head trained to predict zero (the v3 mistake) — realized with the one aux signal (divergence) that generalizes from simulation to real data. Example: LANL CRF02_AG → aux confidence 0.848 (trusted, peaks emitted); a Zaire/Sudan/Bombali cross-species triplet → aux confidence 0.008 (suppressed, OOD warning). The aux head is an out-of-distribution detector, not a learned recombinant classifier — the ML did not (and provably could not) learn the cross-species distinction; the divergence rule makes it. `M3GatedDetector` is also the core of the planned deployment CLI.
