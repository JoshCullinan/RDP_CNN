...
Mirrors cache_test_set.py but for splits/run42_test.txt instead of the
UnseenTestSet directory. Output: cache/ds_pool_run42_test_<key>.npz/.pkl.
...
TEST_LIST = Path('splits/run42_test.txt')
...
def main():
    ...
    print(f"\n=== loading pool/run42_test ({len(paths)} files, max_events={TEST_MAX_EVENTS}) ===")
    X, y, mask, meta = load_filelist_dataset(
        'run42_test', paths, label_mode='breakpoint',
        max_events=TEST_MAX_EVENTS,
    )
    print(f"DONE. X.shape={X.shape} dtype={X.dtype}  meta_n={len(meta)}")

    import glob
    test_caches = sorted(glob.glob('cache/ds_pool_run42_test_*.npz'))
