#!/usr/bin/env python3
"""EB sweep for runB2_7k on UnseenTestSet (full + 11-file subset).

Adapted from eval_runB2_eb_sweep.py. Only the MODEL_PATH and OUT_JSON
differ — same cache, same channel-mask convention, same EBs/thresholds —
so the comparison to the 4000-event runB2 baseline is apples-to-apples.

Headline question: at EB=25 on the 11-file subset, does the 7000-event
model lift sub F1 from runB2's 0.448 by >= +0.01?
"""
...
TOLERANCE = 200
MAX_SEQ_LEN = 32000
MODEL_PATH = 'models_test/cnn_breakpoint_runB2_7k_final.keras'
...
OUT_JSON = Path('results_runB2_7k_eb_sweep.json')
...
def main():
    ...
    assert n_ch == 33, f"expected 33-channel runB2_7k model, got {n_ch}"
    ...
    # Compact 2D table
    print(f"\n{'='*72}", flush=True)
    print(f"  EB SWEEP — runB2_7k on UnseenTestSet (TOLERANCE=200, channels 15:22 zeroed)", flush=True)
    print(f"{'='*72}", flush=True)
    print(f"  {'EB':>5} | {'FULL F1':>8} {'thr':>4} | {'SUB F1':>8} {'thr':>4}  (N_full={len(meta_full)}, N_sub={len(idx_held)})", flush=True)
    ...
    for eb in EBS:
        ...
        marker = '  <-- new "honest" EB' if eb == 25 else ('  <-- prev "honest" EB' if eb == 200 else '')
