...
MAX_SEQ_LEN = 32000
TOLERANCE = 200
MODEL_NAME = sys.argv[1] if len(sys.argv) > 1 else 'models_test/cnn_breakpoint_run42_final.keras'
RUN_TAG = sys.argv[2] if len(sys.argv) > 2 else 'run42'
...
def main():
    ...
    val_npz = pick_pool_cache('run42_val', n_ch)
    val_pkl = val_npz.replace('.npz', '.pkl')
    test_npz = pick_pool_cache('run42_test', n_ch)
