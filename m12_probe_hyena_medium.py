"""M1.2 zero-shot probe — HyenaDNA-medium-160k variant.

Same probe as m12_zeroshot_probe.py but with the medium checkpoint
(`LongSafari/hyenadna-medium-160k-seqlen-hf`, n_layer=8 vs small's 4).
Question: does the deeper variant show meaningful improvement over
HyenaDNA-small's 0.308 mean next-token accuracy on SANTA VAL sequences?

Decision impact (informs M1.2 MLM pretraining direction):
  acc >= 0.40 → size axis matters; consider upgrading backbone
  0.30 <= acc < 0.40 → size axis not the constraint
  acc < 0.30 → unexpected regression; check probe wiring
"""
...
from backbone_hyenadna import v2_to_hyena_ids

MEDIUM_HF_NAME = "LongSafari/hyenadna-medium-160k-seqlen-hf"
...
def evaluate_next_token(model: torch.nn.Module, sequences: list[np.ndarray],
    ...
    for b_start in range(0, n, batch_size):
        ...
        # Free GPU intermediates before the next batch.
        del out, logits, pred, ids
        if device == "cuda":
        ...
        if (b_end // batch_size) % 5 == 0 or b_end == n:
            ...
            if device == "cuda":
                ...
                gpu_str = f"  GPU peak {gpu_peak:.0f} MB"
            print(f"    [{b_end}/{n}]  {rate:.1f} seq/s  RSS {rss:.1f} GB{gpu_str}",
                  flush=True)
...
def main() -> None:
    ...
    ap.add_argument("--hf-name", type=str, default=MEDIUM_HF_NAME)
    ap.add_argument("--out", type=Path,
                    default=Path("m12_probe_hyena_medium.json"))
    args = ap.parse_args()
    ...
    if device == "cuda":
        report["gpu_peak_mb"] = torch.cuda.max_memory_allocated() / 2**20
    ...
    if accs:
        ...
        print(f"\n=== M1.2 medium-variant zero-shot probe summary ===")
        print(f"mean accuracy: {mean_acc:.3f}")
        print(f"(small-32k baseline: 0.308)")
