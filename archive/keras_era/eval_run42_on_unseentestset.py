...
"""
import sys, os, glob
sys.argv = ['eval_run42_on_unseentestset.py',
            'models_test/cnn_breakpoint_run42_final.keras',
            'run42_unseentestset']
...
def pick_pool_val(n_channels_expected):
    pat = 'cache/ds_pool_run42_val_*.npz'
    for p in sorted(glob.glob(pat), reverse=True):
        try:
            with np.load(p) as z:
                if z['X'].shape[-1] == n_channels_expected:
            ...
            continue
    raise FileNotFoundError(f"No pool/run42_val cache with {n_channels_expected} channels")
