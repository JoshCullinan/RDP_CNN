#!/usr/bin/env python3
"""Apples-to-apples: evaluate run #41 on the original UnseenTestSet (33-ch cache).

Run #38's headline 0.313 honest F1 was on this same UnseenTestSet (97 files,
5,539 events, all 25-32k content). Comparing run #41 here removes the
test-set confound from the pooled-split evaluation.
"""
import sys, os, glob
sys.argv = ['eval_run41_on_unseentestset.py',
            'models_test/cnn_breakpoint_run41_final.keras',
            'run41_unseentestset']
...
def pick_pool_val(n_channels_expected):
    pat = 'cache/ds_pool_run41_val_*.npz'
    for p in sorted(glob.glob(pat), reverse=True):
        try:
            with np.load(p) as z:
                if z['X'].shape[-1] == n_channels_expected:
            ...
            continue
    raise FileNotFoundError(f"No pool/run41_val cache with {n_channels_expected} channels")
...
def main():
    ...
    print(f"  val cache (run #41 pooled val): {val_npz}")
    print(f"  test cache (UnseenTestSet, apples-to-apples vs #38): {test_npz}")

    # VAL — for threshold selection
    print(f"\n[{time.strftime('%H:%M:%S')}] VAL eval (pooled run #41 val)")
    d = np.load(val_npz)
    ...
    # TEST = UnseenTestSet (apples-to-apples vs run #38)
    print(f"\n[{time.strftime('%H:%M:%S')}] TEST eval (UnseenTestSet — same set as run #38's 0.313)")
    d = np.load(test_npz)
    ...
    print(f"\n=== APPLES-TO-APPLES vs run #38 (UnseenTestSet, RDP5 raw F1=0.367, run #38 honest=0.313) ===")
    ...
    delta = test_at_val_thr_eb200['f1'] - 0.313
    print(f"  Δ vs run #38: {delta:+.3f}")

    summary = {
        'run_tag': RUN_TAG,
        'model': MODEL_NAME,
        'val_split': 'run41_pool_val',
        'test_split': 'UnseenTestSet (apples-to-apples vs run #38)',
